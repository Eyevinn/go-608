...
- `go608-clock -mode pop-on|paint-on|roll-up[2-4]` (default `pop-on`) selects the caption
  mode, and `-unit-mode default|cue-start|carry` with `-unit-seconds` (default 2) generates
  through the **per-unit API** — one call per unit, as a segment server does — instead of the
  frame-by-frame `Generator`. `cue-start` and `carry` reach the cross-unit policies that exist
